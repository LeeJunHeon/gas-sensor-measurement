...
- **안전 구조** (PLC 래더가 보장)
  ...
  - **하트비트 3초** — 통신이 3초 끊기면 PLC가 통신두절로 트립한다.
    공압 인터록을 제거한 뒤로 이것이 **유일한 자동 트립 조건**이다
    (사유·복원 절차는 [`CONFIG.md`](CONFIG.md) "공압 인터록 제거").
  - ⚠️ **에어 공급 이상은 자동으로 차단되지 않는다** — 운전자가 직접 감시해야 한다.
...
- **배관 모델** — 4-Way는 입구가 둘(혼합가스 / 단독 에어)인 전환 밸브다.
  `VA1`은 가스 매니폴드에 합류하는 **혼합(희석)** 라인, `VA3`은 4-Way로 직행하는
  **단독** 라인이다(`config.json` 의 `route`). 희석 계산과 PURGE는 **혼합 라인만**
  대상으로 하고, 단독 라인은 레시피가 건드리지 않아 사람이 설정한 값을 유지한다
  (안전 차단 시에는 단독 라인도 함께 닫힌다). 자동 실행은 준비 구간 `vent` →
  측정 구간 `sensor`로 4-Way를 스스로 전환하고, 끝나면 `vent`로 돌아간다.
  자세한 내용은 [`CONFIG.md`](CONFIG.md) ③-3.
- DV04A가 1대뿐이라 SV 출력은 4채널이다. 실배선 확정으로 **VA1·VA3·VA5·VA6**이 이를 쓰고
  (밸브 P40~P43, DAC CH0~3, ADC CH0·2·4·5), **VA2·VA4·VA7·VA8은 미배선**이다.
  습도 기능(VA2·VA4)은 배선 전까지 쓸 수 없다 — 레시피의 `rh` 는 0으로 둔다.
  배정표와 증설 절차는 [`CONFIG.md`](CONFIG.md) ③-1·⑤ 참고.
