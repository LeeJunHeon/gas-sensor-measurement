...
```
{
  ...
  //   meas = 코일 ON(Air→Vent·Gas→Sensor).
  //   purge 의 meas 구간은 코일 ON 이므로 에어가 Vent 로 가며 센서는 무풍이다
  //   (의도된 '표 그대로' 동작 — 종류에 따른 4-way 예외는 만들지 않는다).
  // ★ 각 행은 자기 밸브 상태를 완전히 정의한다 —
  //   gas 행: 혼합 배분 + 단독 닫힘 / purge 행: 단독만 + 혼합 닫힘.
  //   전환 겹침·자동 차단 없음(2026-08-14, 이전 '전환 +1s 차단' 계약 폐기).
  //   가스 행 준비 구간은 단독 에어가 닫혀 센서 무풍이다 — 의도된 동작이다.
  //   (레시피 종료 시 전 채널 차단은 안전 규칙이라 그대로 유지한다.)
